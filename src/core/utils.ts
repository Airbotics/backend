 ...
 * Creates an API key
 * 
 * 32 char random string prepended with aak (Airbotics API Key)
 */
export const randomApiKey = () => 'aak_' + randomString();

/**
 * Creates a robot token
 * 
 * 32 char random string prepended with art (Airbotics Robot Token)
 */
export const randomRobotToken = () => 'art_' + randomString();

// create an api key / token
export const createToken = (type: 'robot' | 'api_key') => {

    let plain = '';
    switch (type) {
        case 'robot':
            plain = randomRobotToken();
            break;
        case 'api_key':
            plain = randomApiKey();
            break;
    ...
    const hint = plain.substring(0, 7) + '**************************' + plain.substring(33, 36)
    const hashed = pwdHash(plain);
...
export const pwdHash = (pwd: string) => crypto.pbkdf2Sync(pwd, config.PWD_SALT!, 1000, 64, 'sha512').toString('hex');

// format the short topic name to full mqtt topic
export const formatMqttTopic = (tenantId: string, robotId: string, shortTopic: string) => `${tenantId}/${robotId}/${shortTopic}`; 
...
export const randomName = () => {
    ...
    const third = Math.floor(Math.random() * 99);
